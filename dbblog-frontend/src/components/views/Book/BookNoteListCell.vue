<template>
  <div class="book-note-cell">
      ...
      <iv-col :xs="24" :sm="24" :md="textSpan" :lg="textSpan" :order="textOrderType"
        ...
        <div class="text-wrapper">
          ...
          <p class="operate_info">
            <span class="publish-time"><a>{{ bookNote.createTime}}</a></span>
            <span class="readings"><a><iv-icon type="eye"></iv-icon> {{bookNote.readNum}}</a></span>
            <span class="comments"><a><iv-icon type="compose"></iv-icon> {{bookNote.commentNum}}</a></span>
            <span class="likes"><a ><iv-icon type="heart"></iv-icon> {{bookNote.likeNum}}</a></span>
          </p>
        </div>
      </iv-col>
    ...
    <a class="toggle-arrow" @click="toggleBookInfo" :class="{show: showBookInfo}">
      {{ this.showBookInfo ? '隐藏书籍 ' +  bookNote.book.title : '查看书籍 ' + bookNote.book.title }} &nbsp;<iv-icon type="chevron-up" :class="{show: showBookInfo}"></iv-icon>
    </a>
    <div class="book-infos" :class="{show: showBookInfo}" :href="`book/${bookNote.book.id}`">
